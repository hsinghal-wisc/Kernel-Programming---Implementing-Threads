...
clone(void)
{
	...
	if(argptr(0, (void*)&h, sizeof(h) < 0))
		return -1;
	
	if(argptr(2, (void*)&st, sizeof(st) < 0)){
			return -1;
	}
	int j, pid;
	struct proc *np;
    ...
    *np->tf = *proc->tf;
	...
	np->tf->esp = (uint)st+PGSIZE;
	
	userstack[0] = 0xffffffff;
	userstack[1] = (uint)argument;
	np->tf->esp -= (2)*4;
	copyout(np->pgdir, np->tf->esp, userstack, (2)*4);
	 //Clear %eax so that fork returns 0 in the child.
	np->tf->eax = 0;
    np->tf->ebp = np->tf->esp;
	np->tf->eip = (uint)h;
...
join(void)
{
  ...
  if(argptr(0, (void*)&st, sizeof(st) < 0))
			return -1;
  if((proc->sz-(uint)st)< sizeof(void**))
	  return -1;
